# 🎨 Wamagotchi 에셋 넣는 법 (Phase 2)

이 폴더의 이미지·폰트 **바이너리 파일은 저장소에 포함되어 있지 않습니다**
(용량·라이선스 때문). 아래 안내대로 직접 넣으면 앱에 픽셀 스프라이트와 갈무리 폰트가 적용됩니다.
**파일을 안 넣어도 앱은 정상 실행됩니다** — 스프라이트는 텍스트 얼굴로, 폰트는 시스템 폰트로 자동 대체됩니다.

---

## 1. 정령 스프라이트 (`assets/sprites/`)
