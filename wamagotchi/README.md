...
브랜치에 푸시하면 GitHub Actions(`.github/workflows/build-apk.yml`)가
분석·테스트 후 APK를 빌드해 **릴리스 `wamagotchi-test`** 에 첨부한다.

- **설치/업데이트**: 폰 브라우저로 릴리스 페이지를 열고 `wamagotchi-test.apk`를
  받아 설치. 재설치 시 기존 앱의 **업데이트**로 처리되어 데이터가 유지된다.
- **서명**: 매 빌드 같은 테스트 키(`debug.keystore`, 릴리스에 보관·재사용)로
  서명해 업데이트 설치가 가능하다. 빌드 번호는 CI 실행 번호로 자동 증가.
- ⚠️ 이 키는 공개 저장소에 있는 **테스트 전용** 키다. 구글 플레이 출시 때는
  반드시 새 비공개 업로드 키를 만들어 GitHub Secrets로 관리할 것.
